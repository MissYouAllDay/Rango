...
+ (BOOL) GetYDataFromImage5:(UIImage *)inImage
	...
	NSData *data = (NSData *)CFBridgingRelease(CGDataProviderCopyData(CGImageGetDataProvider([inImage CGImage])));
	pixelData   = (RGBPixel*)[data bytes];
	
	for(int i = 0; i < height; i++) {
		for(int j = 0; j < width; j++) {
			index = i * width + j;
			pixels[i][j] = (pixelData[index].red * 30 + pixelData[index].green * 59 + pixelData[index].blue * 11) / 100;
		}
	}
	
	CFRelease ((CFDataRef)data);
	
	//[self SaveImageBMP:@"test.bmp" data:pixels width:width height:height];
	return YES;
}
...
//	CGImageRef imageRef = CGImageCreateWithPNGDataProvider(imgDataProvider, NULL, true, kCGRenderingIntentDefault);
    ...
    CGContextRef cgctx = [Utility CreateARGBBitmapContext:imageRef];
